...
program `sw/accel/accel.c`**. The program streams Matrix A/B over the system
bus, starts the compute, polls `STATUS`, reads Matrix C back, and writes a result
word into data memory. Unlike the standalone `sim/testbenches/tb_accel.sv` (which
pokes the accelerator's APB slave directly), this exercises the accelerator
through the **real OBI/APB fabric**, end to end.
